      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-white border-b border-slate-100 p-6 flex justify-between items-start shrink-0">
          <div>
             ...
             <h2 className="text-2xl font-bold text-slate-900">{doc.file_name}</h2>
          </div>
            ...
            <section>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {doc.actions.map((action, idx) => (
                  <div key={idx} className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:border-green-200 hover:bg-green-50/30 transition-colors">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-semibold text-slate-800 text-sm">{action.title}</span>
            ...
            <section>
              ...
              <ul className="space-y-3">
                {(doc.key_clauses || []).map((clause, idx) => (
                  <li key={idx} className="text-sm text-slate-600 bg-blue-50/50 p-3 rounded-lg border border-blue-100/50">
                    {clause}
                  </li>
                ))}
              </ul>
            ...
            <section>
              ...
              <ul className="space-y-3">
                {(doc.obligations || []).map((obl, idx) => (
                  <li key={idx} className="text-sm text-slate-600 bg-red-50/50 p-3 rounded-lg border border-red-100/50">
                    {obl}
                  </li>
                ))}
              </ul>
