...
function Work() {
  ...
  const processDocument = async () => {
    ...
    setIsProcessing(true);
    setError(null);
    setLoadingStates({ text: true, summary: true, clauses: true, obligations: true });
    ...
    try {
      ...
      if (extracted && extracted !== "No text extracted") {
        const analyzeResponse = await fetch("http://localhost:5000/analyze_document", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text: extracted }),
        });

        if (!analyzeResponse.ok) throw new Error(analyzeResponse.status === 429 ? "Server busy. Please wait." : "Analysis failed");
      ...
      setLoadingStates({ text: false, summary: false, clauses: false, obligations: false });
      setIsProcessing(false);
    }
  };
      ...
      <header className="sticky top-0 z-40 w-full bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
            ...
            <span className="font-bold text-lg tracking-tight text-slate-800">DocBrief Workspace</span>
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center transition-all hover:shadow-md">
               {!file ? (
                 <>
                   ...
                   <h3 className="text-xl font-bold text-slate-900 mb-2">Upload your document</h3>
                   <p className="text-slate-500 mb-6 max-w-md mx-auto">
                     Drag and drop your file here, or click the button below. We support PDF, PNG, JPG, and TXT files.
                   </p>
                   <input
                    type="file"
                    id="fileUpload"
                 ...
                 <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-slate-50 p-6 rounded-xl border border-slate-100">
                    ...
                    <div className="flex gap-3 w-full sm:w-auto">
                       ...
                       >
                         {isProcessing ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Layout className="w-4 h-4" />}
        ...
        {mode === "compare" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-center mb-10">
              ...
              >
                {isProcessing ? <RefreshCw className="animate-spin" /> : <Split />}
                Run Comparison
              </button>
            </div>
...
const ContentBox = ({ title, icon, content, loading, fullWidth, isCode }) => (
  <div className={`bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden ${fullWidth ? 'col-span-full' : ''}`}>
    ...
    <div className="p-6">
      {loading ? <Spinner /> : (
        <div className={`text-slate-600 leading-relaxed ${isCode ? 'font-mono text-xs bg-slate-50 p-4 rounded-lg' : ''}`}>
           {content}
        </div>
      )}
    </div>
      ...
      {loading ? <Spinner /> : (
        <ul className="space-y-3">
          {items.map((item, index) => (
            <li key={index} className={`text-sm p-3 rounded-lg border ${colorClass}`}>
              {item}
            </li>
          ))}
          {items.length === 0 && <p className="text-slate-400 italic text-sm">No items found.</p>}
        </ul>
      )}
    </div>
  </div>
    ...
    <div className="p-6">
      {loading ? <Spinner /> : items.length === 0 ? (
        <div className="text-center py-6">
           ...
           <p className="text-slate-400 italic text-sm">No deadlines detected.</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {items.map((item, index) => (
            <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-green-200 hover:shadow-md transition group">
...
const UploadDropzone = ({ label, text, onUpload, color }) => (
  <div className={`bg-white p-8 rounded-2xl shadow-sm border-2 border-dashed transition-all text-center ${text ? `border-${color}-500 bg-${color}-50/30` : 'border-slate-300 hover:border-slate-400'}`}>
        ...
        <div>
          ...
          <ul className="space-y-3">
            {result.differences.map((diff, i) => (
              <li key={i} className="text-sm text-slate-700 bg-red-50/50 p-3 rounded-lg border border-red-50">
                {diff}
              </li>
            ))}
          </ul>
        ...
        <div>
          ...
          <ul className="space-y-3">
            {result.similarities.map((sim, i) => (
              <li key={i} className="text-sm text-slate-700 bg-green-50/50 p-3 rounded-lg border border-green-50">
                {sim}
              </li>
            ))}
          </ul>
